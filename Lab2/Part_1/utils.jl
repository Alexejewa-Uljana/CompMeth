...
function uniform_quadratic_knots(a::Float64, b::Float64, n_segments::Int)
    ...
    knots[end-2] = b
    knots[end-1] = b + eps_val
    knots[end] = b + 2*eps_val
    return knots, n, h
end

function bspline_basis(i::Int, d::Int, knots::Vector{Float64}, x::Float64)
    a_actual = knots[3]
    b_actual = knots[end-2]
    if x < a_actual || x > b_actual
        return 0.0
    end
    if d == 0
...
function quad_bspline(x::Float64, i::Int, knots::Vector{Float64})
    return bspline_basis(i, 2, knots, x)
end

function get_points(a::Float64, b::Float64, h::Float64)
    n_segments = Int(round((b - a) / h))
    knots, n, _ = uniform_quadratic_knots(a, b, n_segments)
    return knots
end

function get_coef(f::Function, knots::Vector{Float64}, j::Int)
    t_star = knot_averages(knots, 2)
    n = length(t_star)
    if j == 1
        return f(t_star[1])
    elseif j == n
        ...
        y_curr = t_star[j] 
        y_next = t_star[j+1]
        return (-1/8 * f(y_prev) + 5/4 * f(y_curr) - 1/8 * f(y_next))
    end
end